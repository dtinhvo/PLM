...
No database management is necessary (phew) EXCEPT if you want to use `PlayArtist` (play all songs from an artist) in which case a database creation is the only way - done with a Python script for the tag index (make only, never edited).
